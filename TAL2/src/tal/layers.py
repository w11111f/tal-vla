...
class HeteroRGCNLayer(nn.Module):
    """
    The GCN layer which has been borrowed from DGL. Contains a
    link to the source module.
...
class GatedHeteroRGCNLayer(nn.Module):
    """
    The Gated GCN layer which has been borrowed from DGL.
    Contains a link to the source module.
    https://docs.dgl.ai/_modules/dgl/nn/pytorch/conv/gatedgraphconv.html#GatedGraphConv
    """

    def __init__(self, config, in_size, out_size, etypes, activation):
    ...
    def reset_parameters(self):
        ...
        The model parameters are initialized using Glorot uniform
        initialization and the bias is initialized to be zero.
        """
        gain = init.calculate_gain('relu')
        self.gru.reset_parameters()
        for linear in self.weight.values():
    ...
    def forward(self, G, features):
        ...
        for _ in range(self.config.N_TIMESEPS):
            for etype in G.etypes:
                Wh = self.weight[etype](features)
                G.nodes['object'].data['Wh_%s' % etype] = Wh
                funcs[etype] = (fn.copy_u('Wh_%s' % etype, 'm'), fn.mean('m', 'h'))
...
class LayerNormGRUCell(RNNCellBase):
    ...
    def forward(self, input, hx):
        ...
        return self._LayerNormGRUCell(
            input, hx, self.weight_ih, self.weight_hh, self.ln, self.bias_ih, self.bias_hh,
        )
